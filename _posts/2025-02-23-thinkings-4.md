...
配置好后可以通过单独测试本机和另一台机器的GPU IP保证连通信：
```
# 测试npu之间的连通性：
# 使用 -netdetect 配置当前npu需要检测的另一个npu的地址，然后使用-net_health 来检测是否连接正常
# 如果成功会显示 net health status: Success 错误的话显示 net health status: Receive timeout
# address是目标GPU IP
