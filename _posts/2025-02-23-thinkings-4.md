...
所以如果你也是4台机器每台8卡910B部署 W8A8 的 671B DeepSeek，输入输出设置512tokens的话，也许并发量设置在500是个不错的选择，但是显然并发量1000也可以稳稳撑住。这个测试实际上非常不严谨，因为还受到服务配置的影响，尤其是`maxPrefillTokens`等参数限制后，Prefill阶段Batch其实是会自动分组调度的，后面我再详细测试下不同配置、输入输出token数量差异很大情况下的结果。
